...
Fixture for `tests/bin/cases/audit-laravel-extension.sh`. Composer shape resolves to `api-only` via `bin/audit-laravel-helpers.sh::detect_mode` (non-empty `routes/api.php` + empty `resources/views/`).
