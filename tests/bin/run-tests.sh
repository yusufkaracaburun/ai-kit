...
# Per-agent emitters: aider, cline, continue, cody.
"$AIKIT/bin/emit-rules.sh" "$EMIT_TMP" --rules git-hygiene --agents aider >/dev/null
assert "emit-rules: aider file"    '[ -f "$EMIT_TMP/.aider/rules/git-hygiene.md" ]'
"$AIKIT/bin/emit-rules.sh" "$EMIT_TMP" --rules git-hygiene --agents cline >/dev/null
assert "emit-rules: cline file"    '[ -f "$EMIT_TMP/.clinerules/git-hygiene.md" ]'
"$AIKIT/bin/emit-rules.sh" "$EMIT_TMP" --rules git-hygiene --agents continue >/dev/null
assert "emit-rules: continue file" '[ -f "$EMIT_TMP/.continue/rules/git-hygiene.md" ]'
"$AIKIT/bin/emit-rules.sh" "$EMIT_TMP" --rules git-hygiene --agents cody >/dev/null
assert "emit-rules: cody file"     '[ -f "$EMIT_TMP/.sourcegraph/cody/git-hygiene.md" ]'
assert "emit-rules: aider emitter writes generation marker" \
  'grep -q "Generated by ai-kit" "$EMIT_TMP/.aider/rules/git-hygiene.md"'
...
# Legacy single-file .clinerules is skipped, not clobbered.
EMIT_LEGACY=$(mktemp -d)
: > "$EMIT_LEGACY/.clinerules"
EMIT_LEGACY_OUT=$(mktemp)
"$AIKIT/bin/emit-rules.sh" "$EMIT_LEGACY" --rules git-hygiene --agents cline >"$EMIT_LEGACY_OUT" 2>/dev/null
assert "emit-rules: legacy .clinerules file skipped" 'grep -q "skip:cline" "$EMIT_LEGACY_OUT"'
assert "emit-rules: legacy .clinerules left as a file" '[ -f "$EMIT_LEGACY/.clinerules" ]'
rm -rf "$EMIT_LEGACY" "$EMIT_LEGACY_OUT"
rm -rf "$EMIT_TMP"
