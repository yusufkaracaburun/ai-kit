...
JSON
# Rules emitted by an older ai-kit carry no paths: frontmatter; upgrade must
# re-emit them (#182) and leave a hand-written rule alone.
mkdir -p "$TMP_UP/.claude/rules"
printf '<!-- Generated by ai-kit @ 0.0.1 from standards/rules/laravel-conventions.mini.md -->\n<!-- Mode: always-on. -->\n\n# old copy\n' > "$TMP_UP/.claude/rules/laravel-conventions.md"
printf '# hand-written\nkeep me\n' > "$TMP_UP/.claude/rules/zz-local.md"
printf '<!-- Generated by ai-kit @ 0.0.1 from standards/rules/no-such-rule.mini.md -->\n<!-- Mode: always-on. -->\n\n# gone\n' > "$TMP_UP/.claude/rules/no-such-rule.md"
"$AIKIT/bin/ai-kit-upgrade.sh" "$TMP_UP" >/dev/null
CURRENT_VERSION="$(tr -d '[:space:]' < "$AIKIT/VERSION")"
assert "upgrade re-emits an ai-kit rule with its paths: frontmatter" 'head -2 "$TMP_UP/.claude/rules/laravel-conventions.md" | grep -q "^paths: \[\"app/\*\*\""'
assert "upgrade leaves a hand-written rule alone" 'grep -q "keep me" "$TMP_UP/.claude/rules/zz-local.md"'
assert "upgrade removes a generated rule whose source is gone" '[ ! -f "$TMP_UP/.claude/rules/no-such-rule.md" ]'
