...
# Hand-written golden output for `aikit-to-prd` / `recurring-tasks`

This file represents an *ideal* `aikit-to-prd` response for the `recurring-tasks` fixture.
...
The output below is what a model should produce after running the `aikit-to-prd` skill
...
- A new `recurrence` module owns the repeat-rule definition and the
  materialisation logic. The interface exposes `schedule`, `pause`, `aikit-resume`,
