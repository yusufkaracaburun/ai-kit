...
expects:
  ...
  - does NOT create a branch, run TDD, open a PR, or invoke `aikit-tdd` / `aikit-review` / `aikit-ship` in `dry-run`
  ...
  - surfaces spike status (links the issue + `docs/spikes/aikit-autonomous-ralph.md`) before any destructive step
---

# Prompt

Run `/aikit-autonomous` against this repo. The `ready-for-agent` queue is
