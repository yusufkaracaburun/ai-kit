...
[ -f "$RECOMMEND_TOOLS_SKILL" ] || { echo "Missing: $RECOMMEND_TOOLS_SKILL" >&2; exit 2; }
...
for path in "$EXTERNAL_DIR"/*.json; do
  ...
  # Catalog must be referenced in the scorer lib, the setup skill, OR the
  # recommend-tools skill (companions branch — non-scored, judgement-based).
  ...
  elif grep -q "$name" "$RECOMMEND_TOOLS_SKILL"; then
    echo "OK  $name → wired via recommend-tools/SKILL.md (companions branch)"
  else
    echo "FAIL $name → no wiring path found (add to $SCORER_LIB, setup/SKILL.md, or recommend-tools/SKILL.md)"
    FAIL=1
  fi
done
