...
#
# Sections (added incrementally; only the dead-links check ships in v1):
#   1. dead-links   — markdown `[text](path)` link integrity (relative + repo-absolute paths)
#
# Reports content drift only. Framework health lives in ai-kit-hygiene.sh.
...
usage() {
  cat <<USAGE
Usage: $0 [path] [--skip-dead-links] [--no-prompt]
