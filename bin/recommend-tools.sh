...
usage() {
  cat <<EOF
Usage: $0 <project> [--json] [--kind mcp|hook|plugin|all]

Score MCP servers + Claude Code hook recipes + Claude Code plugins (from
standards/external/{mcp-servers,hooks-patterns,plugins}.json) for relevance
to a target project. Detection runs via bin/detect-tooling.sh; file-presence
...
invokes it for stack-specific MCP + hook + plugin suggestions, then
surfaces the results to the user with the same trust model as
/ai:recommend-rules.
EOF
  exit 1
}
...
case "$KIND" in
  all|mcp|hook|plugin) ;;
  *) echo "--kind must be mcp|hook|plugin|all" >&2; exit 2 ;;
esac
