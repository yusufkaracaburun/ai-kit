...
### Removed
- **11 skills with zero recorded use** (usage log: 1601 events across 40 projects, none for these): `zoom-out`, `show-me`, `ship`, `prototype`, `phase` (the `/ai:phase` slash command stays), `onboard`, `improve-codebase-architecture`, `design-direction`, `contribute-eval`, `audit-architecture-vue`, `audit-architecture-nextjs`. Each cost description tokens in every session's skill index. `autonomous` opens its PR with `gh pr create` directly; the phase-check hook and the `grill-first` rule point Deployment at the project's own release procedure. 44 → 33 skills. Two consumer docs still name `/ai:ship` by hand (emeq-hub `CLAUDE.md`, emeq/system `docs/agents/workflow.md`); emitted rules pick up the new wording on `/ai:upgrade`.
- **The per-skill "Usage logging" section** in 23 SKILL.md files. The plugin's PostToolUse hook has logged every Skill call since it was wired; the sections were dead weight.
- **The committed `workflow/` mirror.** `bin/`, `standards/`, `context/`, `orchestration/` and `CHANGELOG.md` were tracked twice, policed by four `sync-plugin-*.sh` scripts and six structure asserts. The canonical trees now live under `workflow/`; the repo-root paths are symlinks (ADR-0016). One copy, nothing to sync, 26k tracked lines gone. Plugin layout unchanged.

### Fixed
- **`detect-tooling` filed every ai-kit rule as "custom".** `detect_agent_stack` matched `.cursor/rules/*.mdc` basenames against a nine-name list frozen when the emitter stopped using those names, so on emeq 23 of 25 generated rules were reported as hand-written. Classification now keys on the `<!-- Generated by ai-kit ... -->` marker the emitter writes; the list stays for legacy files.
- **`global-rules-link.sh` silently linked nothing when reached through a symlink.** It tested `..` with `[ -d ]` (physical) and then `cd` (logical); the two disagree across a symlink. `pwd -P` throughout.
- README named `handoff` (merged into `checkpoint` in May), pinned `v1.10.0` as the example release, and gave companion counts of 21/25/12 against 24/28/23 in the JSON they cite.

### Added
- **`docs/score.md`**: a five-dimension quality rubric with a measurement table. First measured score 63/100 on 2026-09-19 from four read-only audits; 75 after this release's batch. The 94/100 that circulated in session memory had no rubric and never moved; retired.
- **Release cadence in CONTRIBUTING**: at most one release per working day; a patch ships only after a consumer has run the minor it patches.
- Six audit follow-ups as issues with roadmap rows: #184 marker helper, #185 (closed by this release), #186 catalog prune part 2, #187 gh failures read as absence, #188 dead scripts and wrapper families, #189 test gaps.

## 1.94.2 — 2026-09-19

### Fixed
- **Every patch bump dirtied every generated rule file.** `emit-rules.sh` stamped the kit version into the header of each `.claude/rules/*.md` and `.cursor/rules/*.mdc`, so `/ai:upgrade` 1.93.1 → 1.93.2 in planny-app-mobile changed one comment line in 26 files with no rule change behind it, and polluted a fix-branch worktree. The header is now `<!-- Generated by ai-kit from standards/rules/<name>.mini.md -->`, same as the global variant since v1.94.0; the version lives in `.ai-kit-setup` and nothing read it from the header. The next emit rewrites the header line once per file; after that a rule file only changes when its rule does.
- **`verify-setup.sh` demanded `.cursor/skills` from a project that opted out of the skill merge.** A marker written by `ai-kit-upgrade.sh --skip-skill-merge=true` (v1.79) carries `skip_skill_merge: true` but no `project_skills_merged`, and the fallback derived "merge" from `setup_mode=solo-both`, so `--strict` failed on `.cursor/skills present` in planny-app-mobile (Cursor retired, Claude-only). `skip_skill_merge=true` now counts as `project_skills_merged=false`, per ADR-0012's reading of the retrofit flag.
...
### Changed
- **Universal rules ship through the plugin — `~/.claude/rules/ai-kit`, no per-repo copies (ADR-0015, #171 release 1 of 2).** Three of five ai-kit-marked repos on the reference machine loaded zero universal rules: their owner reverted the 58 files `/ai:setup` wrote ("we hebben ai-kit toch global geïnstalleerd"); the other two carried copies that never received a new rule (#149). Measured today with the #182 headless-sentinel method: Claude Code loads `~/.claude/rules/**/*.md` natively in every project on the machine — pathless files at session start, `paths:`-scoped files on first touch of a matching file, symlinked directories included; subagents get none of it, same as project rules. So the 23 universal always-on rules (17 pathless / 5,239 words, 6 `paths:`-scoped / 2,304) are now pre-emitted into the plugin's `rules/` by `bin/sync-plugin-rules.sh` (`emit_claude_code_global`: `paths:` passthrough, the rule's one-line description in the header, no version stamp — the payload changes when a rule changes, not per release; `--check` drift-tested in the suite and re-stamped by `release.sh`), and a second plugin SessionStart hook, `global-rules-link.sh`, keeps `~/.claude/rules/ai-kit` pointing there with one idempotent `ln -sfn` — the plugin cache path carries the version, so the first session after a `/plugin update` re-points it (that session still ran on the old link; one-session lag, then correct). A real directory at that path is never clobbered. On-demand universals (`code-audit`, `refactoring`, `aposd`, `pragmatic`, `session-coordination`) stay out: nothing under `~/.claude/rules/` can be fenced. `/ai:doctor` reports the link (linked with rule count / dangling / stale after update / real dir in the way / opted out; `info` when no global channel exists at all, skipped for a project-only project — and still reported under the machine-wide `no-globals` opt-out, which is about skill symlinks, not rules), `context-lean` prints the global set as one note line. Opt out machine-wide: `bin/ai-kit-no-global-rules.sh on` (writes `~/.config/ai-kit/no-global-rules`, removes the link; `off` restores it on the next session start). `builder`, `designer`, `reviewer` and `verifier` now list both rule directories (`find … -exec head -4`, so an empty directory on either side never aborts the call under zsh) before their first edit or verdict. The header's `Source:` path is the real one, `standards/rules/feedback/…` for the eight feedback rules — release 2's `/ai:upgrade` keys on it. `CONTRIBUTING.md` and the `harvest` rule-edit recipes gain the `sync-plugin-rules.sh` step. Release 2 (setup/bootstrap: stack rules only per repo, docs scaffold as a recorded choice, `detect-tooling` recognising the ai-kit header, `/ai:upgrade` removing the generated universal copies) follows once the global channel is confirmed live downstream.

### Removed
- **`bin/hooks/session-rules-inject.sh`, `bin/ai-kit-no-rule-injection.sh` and their tests.** ADR-0011 kept the unwired injection hook as a fallback for "a host stops loading `.claude/rules/` natively"; that host would drop `~/.claude/rules/` too, and the project-rules concept with it. ADR-0011 is superseded by ADR-0015; the glossary gains *Global channel*, *Global rules* and *Stack rule*.
...
### Changed
- **Rules on Claude Code: the host loads them, so the emitter now says which ones, when.** Measured 2026-09-19 (CC 2.1.278, headless sentinel probe, #182): Claude Code reads every `.claude/rules/*.md` that has no `paths:` frontmatter at session start — all of them, no budget — and a file with `paths:` only when a matching file is touched. ai-kit's emitter wrote no `paths:`, so every emitted rule was always-on for real: emeq-hub carried 32 files / 14,288 words (≈ 19k tokens) into every session, planny-app-mobile 27 / 12,558 — on-demand audit checklists included. The premise of #144 ("always-on is inert on Claude Code") and the injection hook withheld in #148 no longer hold; the native loader was already spending 7× that budget with no selection. `bin/lib/emitters/claude-code.sh` now emits a rule's own `paths:` (15 always-on stack rules gained one: `laravel-conventions` → `app/**, routes/**, …`, `a11y` → `**/*.tsx, **/*.vue, …`, `sql-style` → `**/*.sql, **/migrations/**`, `design-leads` → `**/*.pen, src/components/**, …`), fences every `default_mode: on-demand` rule behind `paths: [".on-demand/**"]` — never auto-loaded, a skill reads it by path (`code-audit` and its six stack extensions, `aposd`, `refactoring`, `ddd-distilled`, `release-it`, `copy-nl`, …) — and leaves universal always-on rules pathless. Two source invariants are pinned by test: `paths:` is a single-line flow list, and an on-demand rule never declares one. `/ai:upgrade` re-emits the ai-kit rules a project already carries (feedback/, external/ and .nano sources resolve; a generated file whose source is gone is reported as `stale:`; hand-written files untouched), so the frontmatter reaches downstream copies. `context-lean` now counts pathless rules as the always-on tax in two buckets: the kit's universal set as a note (emeq-hub: 25 rules, 10,993 words — a kit decision, not scored against the project) and everything else pathless as the project's own, warned above 2,000 words (emeq-hub: 4 rules, 2,489 → gone after `/ai:upgrade`). ADR-0011 status, the emitters README, `phase-check.sh`, the setup skill and the glossary now describe the structural behaviour; the injection hook stays in the tree unwired as a fallback and strips frontmatter properly should it ever run.
- **Subagents read the rules they need — nothing else delivers them.** The same probe showed a subagent receives CLAUDE.md, memory and its AGENT.md, never `.claude/rules/` and (unless `tools: All`, ≈ 70k tokens of listings per spawn) no Skill tool. `builder`, `designer`, `reviewer` and `verifier` gained one concrete step: one `head -3 .claude/rules/*.md` for the `paths:` map, then `Read` only the rules whose `paths:` cover the files in scope, and for the pathless ones only the `docs/agents/active-rules.md` rows that apply — never the directory wholesale. A reviewer treats a violated rule as a finding; a verifier treats a claim that holds only by ignoring one as REFUTED.
...
### Added
- **`design-to-code`: the second harvest — what two more design-first repos learned, filtered.** Assessed a 208 KB project pencil skill (23 files, artefact-derived parity ledger, expiring exemptions, contract-aligned build) and a user-scope `design-check` skill (a stale fork of a fidelity script) against the kit; took ten mechanisms as short rules, left the stack (scripts, rule ids, MSW seam, node ids), the dead (an eval set with no runner, vendored Pencil docs, 19 files naming MCP tools that no longer exist) and the already-covered (loose copies, one screen at a time, never Read a .pen). Audit: the design is audited for what the code will need — states drawn per interactive element, a frame for every overlay the code opens (grep the code for dialog/sheet/confirm), contrast and touch targets; who fixes what (drift and below-bar you fix; a gap or a token-changing contrast failure you propose); never write a design string from memory; a recurring measurement error becomes code in the project's check, not a note in the skill. Build, three rules two repos each learned the hard way: spec table before the first line of code (a 22/800 heading ships as 17/600 from a text dump), frame proposes / contract disposes (a field the API lacks is mock-only plus an issue, never an invented column; UI-only screens exempt), rebuild means clean slate. Prove: a golden or self-baseline compares the app with itself and can never show app ≠ design; colour is the leg that slips — assert it, and pick a variant by the node's token, never its name; a test entry (query param, flag, fixture) may swap data and state, never styling. Copy, because a text gate compares literal substrings: frame and code in the same pass, identical string; placeholder text in the map is a literal copy; text inside a master re-runs every instance's check, so accept-with-reason is part of a copy pass; the review surface for a copy pass is the design file's diff plus a change list, not a sibling frame per line (field report from a 121-edit copy pass the same day). Done is derived, not declared: the code names its frame where a grep finds it, the passed check is pinned to the frame's hash, an exemption has a reason, an owner and an expiry ≤ 30 days — "a permanent exemption is a lie with a config key". The `designer` subagent carries the three build rules.
- **`pencil-sweep.md`: five verified Pencil edges that fail silently.** `BROKEN-REF` in the promote sweep — an instance whose master was deleted collapses to 1px or an empty box and the export draws a red hatch a raster tolerance hides (one sat in a baseline for three months; not `Update`-able, `Replace` it); read tokens with `resolveVariables:false` or raw hex looks tokenized; trailing dead space (last child's bottom ≠ inner height); exports go stale silently (re-export in a second call after Insert/Move, fractional container position exports one row too tall, a module export ages when an imported library changes, `placeholder:true` renders blank, only a full tab reopen reads disk); and "is the file on disk what the app shows?" — compare both ways before building or hashing; the app has been seen not flushing MCP edits for a session and, two weeks later, writing within seconds — never assume which side is behind.
- **`design-leads` rule:** done is derived from artefacts (code→frame pointer, hash-pinned check, expiring exemption), paired with the canvas marker.

## 1.89.0 — 2026-09-18

### Added
- **`design-leads` rule — the design file is the source, code follows, and the bar for "matches" is a verdict, not a glance.** Harvested from a Flutter app that gated 39 screens this week and an Astro site that gates 30 pages mechanically, both of which arrived at the same shape independently: a fixed tie-break (drift is fixed frame-first, never by editing the design to match code), a not-drift list (sample data, states the frame does not depict, platform copy a single-OS frame cannot carry), one screen → self-review → **user OK** → next (the OK is what stops building ahead of the phase — one repo built ahead three times without it), never 100 with an open finding, and a done-marker on the canvas itself (✅ in the label + a node with the source path) because a status table in a doc drifts. Plus the design-editing discipline that kept the file wired: instance over loose copy, master before a repeated raw structure, a component's debut on a real screen is a test not a wiring step (no instance-only overrides), proposals as a sibling frame at true size, token edits read back by exact name. Non-universal; surfaces via the new `pencil` framework signal (`*.pen` at the repo root or under `.pencil/`, `bin/lib/detect-lib.sh`), so `/ai:recommend-rules` offers it exactly where a design file exists.
- **`design-to-code`: the project overlay is now a contract, the sweep ships as scripts, and the prove step carries the evidence of which check finds what.** New "project overlay" section names what only the project knows and a build must not have to ask for mid-unit: frame → route and master → component maps, a *state recipe* per screen (which account/record/time the frame shows and how to reach it — reaching the state is the cost of the live compare, not the screenshot), the undesigned states, copy per locale, interactions, sweep parameters. The `designer` subagent takes those as inputs and reports a missing one under "Noticed, not done" so the overlay gets fixed, not the next build. `pencil-sweep.md` carries the structural sweep and the promote sweep (LOOSE-COPY / RAW-REPEAT) as parametrised Pencil `execute` scripts, the PNG/`html-css` export idioms (no SVG target; `Generate`-SVG marks can render blank), and a ~15-line node variant for a plain-JSON `.pen` — as an audit, never a commit gate. Prove now weights its steps by measured yield (a 39-screen re-gate: sweep 0, full-scale render 0, live on two densities 22 drifts + 2 bycatch bugs → sweep always, render only after `.pen` edits, live never skipped), adds the web variant (look at two viewports; a snapshot diff goes structurally red across machines), and fixes the shape of a multi-screen prove: read-only sweep agents → `verifier` per claim before anyone edits (2 of 30 claims named the wrong field) → a fix batch that never commits → one committer. Batch table gains `sweep · live ×2 · drift found → fixed · in code (path)` and is shown in the chat.
- **`modern-web-guidance` catalog entry (`plugins.json`, category `web-platform`).** Chrome + Edge team's eval-graded implementation guides (143 in v0.0.189), retrieved on demand through a local semantic search — the "how to build it with the platform" counterpart to `ui-ux-pro-max` (what to choose) and `web-quality-skills` (measure afterwards), a slot the catalog did not have. `/ai:should-i-use` 2026-09-19, full eight-criteria pass in `VETTING.md`; two caveats carried into the entry: export `DISABLE_TELEMETRY=1` first (a detached watchdog posts search queries + guide ids to Google), and the skill's own "execute FIRST for all HTML/CSS" trigger is aggressive — the first repo to enable it measures the token cost. `design-to-code` build order and the `designer` subagent name it as the web platform-guidance source in slot 3; it answers "how", never "what".

### Changed
- **`semver` rule:** the commit that changes user-visible behavior writes its own `## [Unreleased]` line — the bump only dates the section, and the store text is that section shortened; artifacts with a build number tag version *and* build (`v1.6.2+61`), monotonic across versions.
- **`session-coordination` rule:** sessions sharing files work in their own `.agents/worktrees/<name>`; in a shared checkout non-leads hand their diff to the lead. Generated artifacts (goldens, exports) are regenerated once at the end of a batch by the committer, not per commit per session.
- **Counts:** rules 44 → 45 everywhere `count-primitives --check` looks; README / ONBOARDING catalog counts caught up with the JSON (24 MCP servers, 28 hook recipes, 23 plugins — they said 21 / 25 / 12). `standards/external/VETTING.md` crosses 1000 lines with this entry; it is an append-only audit ledger nothing loads whole (tests grep it), so it stays one file.
