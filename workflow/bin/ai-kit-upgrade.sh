...
# Re-emit the Claude Code rules the project already carries, so an emitter
# change (paths: frontmatter, #182) reaches the emitted copies — only files
# generated by ai-kit whose source still exists; hand-written files in
# .claude/rules/ are left alone. Names follow emit-rules.sh: bare for
# standards/rules/ and feedback/, `external/<slug>` for the external subdir.
if [ -d "$TARGET/.claude/rules" ]; then
  reemit=""
  while IFS= read -r f; do
    head -5 "$f" | grep -q "Generated by ai-kit" || continue
    name="${f#"$TARGET/.claude/rules/"}"; name="${name%.md}"
    found=""
    for cand in "$name" "feedback/$name"; do
      for ext in mini.md nano.md; do
        [ -f "$AIKIT/standards/rules/${cand}.${ext}" ] && { found=1; break 2; }
      done
    done
    # Generated by ai-kit, source gone (rule removed or renamed — a rename is
    # re-emitted under its new name): a pathless leftover would load every
    # session forever, so it goes. Hand-written files never reach this line.
    [ -n "$found" ] || { rm -f "$f"; echo "removed: .claude/rules/${name}.md — generated by ai-kit, source no longer in standards/rules"; continue; }
    reemit="${reemit:+$reemit,}$name"
  done < <(find "$TARGET/.claude/rules" -name '*.md' -type f | sort)
  if [ -n "$reemit" ]; then
    echo ""
    echo "Re-emitting $(printf '%s' "$reemit" | tr ',' '\n' | wc -l | tr -d ' ') Claude Code rule(s) against v${VERSION}…"
