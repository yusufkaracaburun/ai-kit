...
if [ -d "$TARGET/.claude/rules" ]; then
  ...
  while IFS= read -r f; do
    ...
    [ -n "$found" ] || { echo "stale: .claude/rules/${name}.md — generated by ai-kit, but no source in standards/rules (renamed or removed); left as is, still loads every session"; continue; }
