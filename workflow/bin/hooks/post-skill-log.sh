...
# Locate log-skill.sh. Two valid layouts ship today:
#   1. bin/hooks/post-skill-log.sh  + bin/log-skill.sh        (symlink/source install)
#   2. workflow/hooks/post-skill-log.sh + workflow/hooks/log-skill.sh (plugin install)
# Try the sibling location first (plugin), fall back to one dir up (source).
SCRIPT_HERE="$(cd "$(dirname "$0")" && pwd)"
if [ -x "$SCRIPT_HERE/log-skill.sh" ]; then
  LOG_SH="$SCRIPT_HERE/log-skill.sh"
elif [ -x "$SCRIPT_HERE/../log-skill.sh" ]; then
  LOG_SH="$SCRIPT_HERE/../log-skill.sh"
else
  exit 0
fi

"$LOG_SH" "$skill" "done" >/dev/null 2>&1 || true
exit 0
