# Architecture setup guide (for /aikit-setup skill)
...
- **Greenfield + user wants structure** → offer at most 2 options with tradeoffs; suggest ADR for the chosen approach via `aikit-grill-with-docs`.
...
3. Cross-cutting: monorepo boundaries, where new features go, pointer to `aikit-improve-codebase-architecture` and `aikit-grill-with-docs`.
