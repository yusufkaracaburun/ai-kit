...
**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling) hand off to the `/ai:improve-codebase-architecture` skill with the specifics. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.
...
```bash
bash "$AI_KIT_ROOT/bin/log-skill.sh" diagnose start  # at the start
bash "$AI_KIT_ROOT/bin/log-skill.sh" diagnose done   # at the end (or `abort` if you bail)
```
