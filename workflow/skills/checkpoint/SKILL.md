...
`/checkpoint` is one of three related skills. Pick the right one before
writing anything:

- **`/checkpoint`** (this skill) — writes to the user's auto-memory folder
  (`~/.claude/.../memory/project/`). For *future you* in the same project.
  Pairs with `/resume`. Persists across sessions on the same machine.
- **`/handoff`** — writes to `$TMPDIR/handoff-*.md`. For another agent,
...
- **Memory folder missing**: tell the user the auto-memory system is not
  initialised for this project — `/checkpoint` only makes sense when there
...
- **Open / next** — what was about to be done when the user invoked
  /checkpoint. Be specific: "T-5.6 enrollments port — start by reading
...
Do not delete stale lines — `/resume` skips them but they remain visible
...
Next session: /resume — or read that file + MEMORY.md and pick up from
"Open / next".
