...
5. **Next cycle**
   ...
   - Offer to run `aikit-to-issues` if user wants tickets created
...
Works without fixed sprint length — Kanban teams run `/aikit-retro` on a rhythm they choose.
...
```bash
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-retro start  # at the start
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-retro done   # at the end (or `abort` if you bail)
```
