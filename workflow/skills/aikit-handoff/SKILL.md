...
```bash
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-handoff start  # at the start
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-handoff done   # at the end (or `abort` if you bail)
```
