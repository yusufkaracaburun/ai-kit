...
```bash
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-to-issues start  # at the start
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-to-issues done   # at the end (or `abort` if you bail)
```
