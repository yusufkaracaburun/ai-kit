...
2. If already installed (Phase 1 detection), skip straight to appending the AGENTS.md pointer.
3. Otherwise offer whichever tier(s) fired. On yes: `npx skills@latest add emilkowalski/skills -a claude-code -y -s <comma-separated skill names for the tier(s) that fired>` — `animate,review-animations,improve-animations,find-animation-opportunities,animation-vocabulary` for the animation tier, `ask-sonner` for the sonner tier, both combined with a comma if both fired. Never pass `--all` or omit `-s` — the upstream repo also ships `write-swift`/`animate-expo`/`apple-design`/`mobile-native`/`pick-ui-library`/`prototype`/`emil-design-eng`, none of which are wired here (see companions.json install notes for why).
