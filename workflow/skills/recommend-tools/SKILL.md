...
| **scrapling** | Web-fetch — reaching pages WebFetch/browser tools can't | Official MCP server: multi-page/session crawling, JS-render, anti-bot bypass (Cloudflare Turnstile), prompt-injection sanitization. Feeds design reference research and API-doc ingestion. | Low — clean VETTING.md pass, BSD-3-Clause, no caveats. |
...
| **emilkowalski-skills** | Design — animation-specific taste (easing/duration/physicality) that ui-ux-pro-max/impeccable/skillui don't cover | Six markdown skills from Sonner/Vaul's author: `animate` (build motion against a strict decision table), `review-animations` (critique a diff), `improve-animations` (read-only audit-then-plan workflow, one plan per finding), `find-animation-opportunities`, `animation-vocabulary` (reverse-lookup glossary), and `ask-sonner` (gated separately on the `sonner` dependency). | Low — plain markdown, no executable code; installed via the official Vercel `skills` CLI. |
...
- **`ADOPT`** — present, useful, not in any catalog. File it via `/ai:to-prd` Defer mode with label `catalog-candidate` to queue it for VETTING.md review. Mention to the user that this is a promotion candidate, not a current recommendation.
...
[ -d graphify-out/wiki ] && echo "graphify: wiki tier present" || { graphify --help 2>/dev/null | grep -q "export wiki" && echo "graphify: wiki tier not generated (export wiki available)" || echo "graphify: wiki tier unavailable — this graphify has no 'export wiki'"; }
...
{ [ -d "$HOME/.claude/skills/animate" ] || [ -d .claude/skills/animate ]; } && echo "emilkowalski-skills: installed" || echo "emilkowalski-skills: not installed"
{ [ -f package.json ] && grep -qE '"(react|vue|@angular/core|next|nuxt|svelte|astro|react-native)"' package.json; } && find . -type d \( -name node_modules -o -name .git -o -name vendor \) -prune -o -type f \( -name '*.tsx' -o -name '*.jsx' -o -name '*.vue' -o -name '*.astro' -o -name '*.css' \) -print 2>/dev/null | xargs grep -lE 'transition-|animate-|@keyframes|framer-motion|useAnimat' 2>/dev/null | head -1 | grep -q . && echo "emilkowalski-skills: animation tier — project has real motion code" || echo "emilkowalski-skills: animation tier — no motion signal in this repo"
{ grep -q '"sonner"' package.json 2>/dev/null || find . -maxdepth 3 -name package.json -not -path '*/node_modules/*' -exec grep -l '"sonner"' {} \; 2>/dev/null | head -1 | grep -q .; } && echo "emilkowalski-skills: sonner tier — sonner dependency present" || echo "emilkowalski-skills: sonner tier — no sonner dependency"
```
...
- **emilkowalski-skills** — two independent gates, offer per-tier: the **animation** tier (animate/review-animations/improve-animations/find-animation-opportunities/animation-vocabulary) fits any project with a frontend framework and real transition/animate/`@keyframes`/framer-motion code — weak fit with no motion signal at all. The **sonner** tier (ask-sonner) fits whenever the `sonner` package is a dependency, independent of the animation gate — a backend-heavy app that only uses Sonner for toasts still gets it. Neither overlaps ui-ux-pro-max/impeccable/skillui/inspo-mcp — those are palette/token/anti-slop/reference-site tools, none cover animation-specific taste (easing, duration, physicality, interruptibility). Do not offer `emil-design-eng` from the same upstream repo — it duplicates `animate` + `review-animations`' content; do not offer `write-swift`/`animate-expo`/`apple-design`/`mobile-native`/`pick-ui-library`/`prototype` either — no confirmed signal for them yet (see companions.json install notes for what was checked and ruled out).
...
5. Tell the user to run `graphify .` (or the upstream's init command) once to build `graphify-out/`, then `graphify update .` after code changes. Say the cost split out loud: `graphify <path>` is a full semantic run (LLM extraction + community labeling — tokens, minutes); only `graphify update <path>` is AST-only and free. Document corpora (`.docs/`, `raw/`, scraped Markdown) go in `.graphifyignore` first, or the next `graphify .` re-extracts every page through the LLM.
6. **Wiki tier (opt-in).** If `graphify-out/` exists but `graphify-out/wiki/` does not, and Phase 1 saw `export wiki available`, mention the opt-in tier: `graphify export wiki` writes `graphify-out/wiki/index.md` plus one Markdown article per node from the existing `graph.json` (no LLM, seconds). One article per node: a graph that includes documents yields an article per heading. Useful when the agent does many symbol-lookup grep'ing. Decision rule: recommend only when the user explicitly asks for it, the repo is large enough that `query` results scroll past a page, or the recent transcript shows >3 grep/find calls for symbol locations. Otherwise skip — `query`/`path`/`explain` cover the common case. Companion catalog entry: `standards/external/companions.json` → `graphify.tiers.wiki`.
...
**scrapling:**
1. Offer when the project does external research, third-party API-doc ingestion, or reference-site work (design references, llm-wiki) — or simply when the user reports WebFetch/browser tools missing pages.
...
**emilkowalski-skills:**
1. Evaluate the two tiers independently — a project can get one, both, or neither. Animation tier needs Phase 1's motion signal; sonner tier needs the `sonner` dependency alone.
2. If already installed (Phase 1 detection), skip straight to appending the AGENTS.md pointer.
3. Otherwise offer whichever tier(s) fired. On yes: `npx skills@latest add emilkowalski/skills -a claude-code -y -s <space-separated skill names for the tier(s) that fired>` — `animate review-animations improve-animations find-animation-opportunities animation-vocabulary` for the animation tier, `ask-sonner` for the sonner tier, space-separated and concatenated if both fired (a comma-joined list is read as one unknown name and installs nothing). Never pass `--all` or omit `-s` — the upstream repo also ships `write-swift`/`animate-expo`/`apple-design`/`mobile-native`/`pick-ui-library`/`prototype`/`emil-design-eng`, none of which are wired here (see companions.json install notes for why).
4. Append `companions/emilkowalski-skills.md` to the project `AGENTS.md` — the pointer note, scoped to whichever tier(s) were wired.
