...
Then route to the next skill: usually [to-prd](../aikit-to-prd/SKILL.md), [to-issues](../aikit-to-issues/SKILL.md), or directly to writing an ADR.
...
```bash
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-grill-me start  # at the start
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-grill-me done   # at the end (or `abort` if you bail)
```
