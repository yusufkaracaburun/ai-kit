---
name: aikit-resume
description: "Find the most recent /aikit-checkpoint memo for the current project, summarise it, and propose the first concrete action — pick up where you left off. Use at the start of a fresh session, after /clear or /compact, when the user asks 'where was I?', wants to continue previous work, or resume after a break."
allowed-tools:
  - Read
  - Bash
  - Grep
...
need to re-acquire context from the previous session's `/aikit-checkpoint` memo.
...
files, stop and tell the user — `/aikit-resume` only makes sense when there is
a checkpoint to read. Suggest `/aikit-checkpoint` if they want to start the
discipline now.
