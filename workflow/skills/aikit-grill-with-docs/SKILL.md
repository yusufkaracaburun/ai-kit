...
### Update architecture.md for structure decisions
...
```bash
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-grill-with-docs start  # at the start
bash "$AI_KIT_ROOT/bin/log-skill.sh" aikit-grill-with-docs done   # at the end (or `abort` if you bail)
```
