...
Confirm the marker version moved to the expected value (`jq -r .ai_kit_version .ai-kit-setup`). If the script fails with "No .ai-kit-setup", the project isn't bootstrapped — point the user at `bin/bootstrap-project.sh` and `/ai:setup` instead.
