...
flowchart LR
  subgraph SRC["Source layer — ai-kit repo (single source of truth)"]
    SA["workflow/skills · agents<br/>44 skills + 6 subagents"]
    CB["workflow/commands + bin/ai-kit-*.sh<br/>11 slash commands + CLI"]
    H["bin/hooks/*.sh + hooks.json<br/>2 hook wirings"]
    R["standards/rules/*.mini.md<br/>44 agent-agnostic rules"]
  end
...
flowchart TD
  ...
  DEL -->|yes| SUB["Subagent in its own context<br/>reviewer · qa-runner · explore · verifier · builder · designer"]
