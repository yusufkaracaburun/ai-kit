# ADR-0003: Merge `security-audit` into `review`
...
v0.1 shipped `security-audit` as a separate skill alongside `review`. In practice they were invoked together 80% of the time, and `security-audit` mostly re-implemented `review`'s diff-walking logic against a security-specific rubric. Two skills, one workflow, lots of duplication.
...
Fold the security-audit rubric into `review` as a **deep pass**:

- `review` runs the standard pre-merge check (correctness, tests, alignment with CONTEXT.md).
...
- One mental model: pre-merge = `review`.
...
- A user who wants *only* a security audit (e.g. of a stale branch they're not about to merge) has to invoke `review` and ignore the non-security findings. Rare enough to accept.
