...
3. ai-kit can fully complete `/setup` without ever running the thing.
...
- **Don't hard-fail when the plugin is absent.** If the user picked `completed` but doesn't have the plugin installed, that's their problem — `/setup` still exits clean.
