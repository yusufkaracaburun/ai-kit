...
`standards/rules/*.mini.md`; per-agent emitters translate it into each agent's
conventional directory (`.cursor/rules/`, `.claude/rules/`, `.continue/rules/`,
`.clinerules/`, `.aider/rules/`, `.sourcegraph/cody/`). Every emitted file carries a
...
**Multi-agent bonus.** If you adopted Aider / Cline / Continue / Cody since the pre-2.0
install, step 2 populates their rule directories too — no separate action needed.
