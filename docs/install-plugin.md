...
After install, Claude Code namespace-prefixes plugin skills: `/ai:setup`, `/ai:ship`, `/ai:tdd`, etc. Slash commands likewise get the prefix: `/ai:doctor`. Subagents (`explore`, `reviewer`, `qa-runner`, `verifier`) become discoverable via the Task tool.
...
- All 4 subagents (`workflow/agents/explore`, `workflow/agents/reviewer`, `workflow/agents/qa-runner`, `workflow/agents/verifier`)
