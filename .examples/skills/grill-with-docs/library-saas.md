...
- **No re-derivation across sessions.** Once written, this CONTEXT.md is the cached truth — future skills (`/ai:review`, `/ai:improve-codebase-architecture`) read this *before* touching code.

This is the bar. Other `grill-with-docs` showcases — and `to-prd`, `review`, `improve-codebase-architecture`, etc. — should produce equally concrete, equally domain-specific output.
